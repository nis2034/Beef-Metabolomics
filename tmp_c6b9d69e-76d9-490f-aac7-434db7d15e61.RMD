...
*Log text:*<br/>GeneNet partial correlation

WARNING: Large data frame (174345 rows). Displaying first
1000 rows.
```{r results='asis'}
# extract result table
df<-r[[44]]$output$table
# add feature names
rd <- rowData(D)
df <- cbind(name=as.data.frame(rd)$name[match(df$var, rownames(rd))], df) %>%
  dplyr::arrange(p.value)
...
### post_multtest
*Log text:*<br/>Multiple testing correction of 'GGM' using 'BH'


### plots_net
*Log text:*<br/>Correlation Network, aes: GGM
